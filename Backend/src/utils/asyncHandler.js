...
const asyncHandler = (requestHandler) => {
      return (req, res, next) => {                           // Returns a function to wrap the provided request handler
                                                           // Wrap the request handler in a resolved promise to catch any asynchronous errors
          Promise.resolve(requestHandler(req, res, next))
               .catch((err) => next(err));                 // Passes any errors to the next middleware (error handling middleware)// Explanation:// - `req`: The request object containing client request data // - `res`: The response object to send data back to the client // - `next`: A function to pass control to the next middleware or error handler
     }
